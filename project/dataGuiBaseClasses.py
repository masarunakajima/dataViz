...
def preparePlot(p,data,param,**keys): #prepares plot for first time or with some dataItems existing in plot
    param.reset_index(inplace=True,drop=True)
    l = len(p.dataItems)
    if l==0:
        p.addLegend()
        p.legend.setScale(keys['legendScale'])
    pens=keys['plotPenList']
    nPens=len(pens)
    for i in range(len(param)):
        para = param.iloc[i].copy()
        y = data[para[keys['channelIdKey']]]
        ...
        #calibration and unit 
        if para[keys['isUncalibratedSignalKey']]:
            yUnit = para[keys['unitKey']][0]
        else:
            y = y*float(para[keys['calibrationKey']])            
            yUnit = para[keys['Y_Unit_LabelKey']]
        
        #smooth
        if para[keys['isSmoothKey']]:
            if para[keys['nSmoothKey']].isdigit():
                if 0<int(para[keys['nSmoothKey']]) and int(para[keys['nSmoothKey']]) <len(y):
                    y = smooth(y,int(para[keys['nSmoothKey']]))
        
        #dc cancel
        if para[keys['isDcCancelHeadKey']]:
            if para[keys['nDcCancelHeadKey']].isdigit():
                if 0<int(para[keys['nDcCancelHeadKey']]) and int(para[keys['nDcCancelHeadKey']]) <len(y):
                    y = y - np.average(y[:int(para[keys['nDcCancelHeadKey']])])
        elif para[keys['isDcCancelTailKey']]:
            if para[keys['nDcCancelTailKey']].isdigit():
                if 0<int(para[keys['nDcCancelTailKey']]) and int(para[keys['nDcCancelTailKey']]) <len(y):
                    y = y - np.average(y[-int(para[keys['nDcCancelTailKey']]):])      
        # create x array       
        x = float(para[keys['X0Key']])+float(para[keys['Delta_XKey']])*np.arange(len(y))
        name = '%s : %s' %(str(para[keys['shotNumKey']]),str(para[keys['channelDescriptionKey']]))
        p.plot(x,y,name=name[:keys['nStrLegend']],pen=pens[len(p.dataItems)%nPens])
    
    
    if l ==0:
        # set axis preferences  
        axes =[p.getAxis('left'),p.getAxis('bottom')]
        axisArg = keys['axisArgs'] 
        p.getAxis('left').setLabel('',units = param.iloc[0][keys['unitKey']],**axisArg['labelStyleArgs'])
        p.getAxis('bottom').setLabel(param.iloc[0][keys['X_DimensionKey']],units = 's',**axisArg['labelStyleArgs'])        
        for i in range(2):
            axes[i].tickFont = axisArg['tickFont']
            axes[i].setPen(axisArg['axisPen'])
            axes[i].setStyle(tickLength = axisArg['tickLength'])
            if i==0:
                axes[i].setWidth(w=axisArg['yAxisWidth'])
                axes[i].setStyle(tickTextOffset = axisArg['yTickTextOffset'])
            else:
                axes[i].setHeight(h=axisArg['xAxisHeight'])
                axes[i].setStyle(tickTextOffset = axisArg['xTickTextOffset'])
...
#different type of functions pf(processing functions)


    
    
class PlotDataItem1D (pg.PlotDataItem):
    """
    plotdataitem with an additional attributes 
    plotType: type of plot data (str) 
    dataParam: daq data param (df)
    processParams: data processing parameters (dict)
    processFunction: function for processing the data and outputting the processed data(function)
    updateData: method:  use the function and param
    
    """
    plotTypes = ['basic_Plot','psd_Plot']
    
    def __init__(self, plotType='basic_Plot',rawData=pd.DataFrame(),dataParam=pd.DataFrame(),processParam={},*args, **kargs):
        ...
        self.xUnit = ''
        self.yUnit = ''
        if self.plotType=='basic_Plot':
            self.processFunction = self.processFunctionBasic
        elif self.plotType == 'psd_Plot' :
            self.processFunction = self.processFunctionPsd
        if len(self.rawData)!=0:            
            self.updateData1D(*args,**kargs)
        
    def updateData1D(self, *args, **kargs):        
        self.processFunction(*args,**kargs)

        
    def processFunctionBasic (self, *args,**kargs):
        
        para = self.dataParam.reset_index(drop=True).iloc[0]

        y = np.array(self.rawData[para[kargs['channelIdKey']]])
    
        
        #calibration and unit 
        if self.processParam[kargs['isUncalibratedSignalKey']]:
            yUnit = para[kargs['Y_Unit_LabelKey']]
        else:
            y = y*float(para[kargs['calibrationKey']])            
            yUnit = para[kargs['unitKey']]
        
        #smooth
        if self.processParam[kargs['isSmoothKey']]:
            if self.processParam[kargs['nSmoothKey']].isdigit():
                if 0<int(self.processParam[kargs['nSmoothKey']]) and int(self.processParam[kargs['nSmoothKey']]) <len(y):
                    y = smooth(y,int(self.processParam[kargs['nSmoothKey']]))
        
        #dc cancel
        if self.processParam[kargs['isDcCancelHeadKey']]:
            if self.processParam[kargs['nDcCancelHeadKey']].isdigit():
                if 0<int(self.processParam[kargs['nDcCancelHeadKey']]) and int(self.processParam[kargs['nDcCancelHeadKey']]) <len(y):
                    y = y - np.average(y[:int(self.processParam[kargs['nDcCancelHeadKey']])])
        elif self.processParam[kargs['isDcCancelTailKey']]:
            if self.processParam[kargs['nDcCancelTailKey']].isdigit():
                if 0<int(self.processParam[kargs['nDcCancelTailKey']]) and int(self.processParam[kargs['nDcCancelTailKey']]) <len(y):
                    y = y - np.average(y[-int(self.processParam[kargs['nDcCancelTailKey']]):])      
        # create x array       
        x = float(para[kargs['X0Key']])+float(para[kargs['Delta_XKey']])*np.arange(len(y))
        xName = para[kargs['X_DimensionKey']]
        xUnit = ''
        if xName.lower()=='time':
            xUnit = 's'    
        yName = para[kargs['channelDescriptionKey']]
        
        self.setData(x,y, name=yName[:20])        
        self.xUnit = xUnit
        self.yUnit = yUnit
        self.xName = xName
        self.yName = yName
    ...
    def processFunctionPsd (self, *args,**kargs):
        

        para = self.dataParam.reset_index(drop=True).iloc[0]
        s = np.array(self.rawData[para[kargs['channelIdKey']]])
        t = float(para[kargs['X0Key']])+float(para[kargs['Delta_XKey']])*np.arange(len(s))

        nperseg = self.processParam[kargs['npersegKey']]
        fs = float(1/(t[1]-t[0]))
        
        #calibration and unit 
        if not self.processParam[kargs['isUncalibratedSignalKey']]:
            s = s*float(para[kargs['calibrationKey']])     
        
        if kargs['xRegionKey'] in self.processParam:
            xRegion = self.processParam[kargs['xRegionKey']]
        ...
        if self.processParam[kargs['isNormalizePsdKey']]:
            s = s/np.average(s)
        
        f,psd = welch(x = s, fs = fs, nperseg = nperseg)
            

        if para[kargs['X_DimensionKey']].lower()=='time':
            xName = 'Frequency'
            xUnit = 'Hz'
        else:
            xName = ''
        ...
        self.setData(f,psd, name=para[kargs['channelDescriptionKey']][:20])        
        self.xUnit = xUnit
        self.yUnit = yUnit
        self.xName = xName
        self.yName = yName
...
def tableFindOccupiedCells(table):
    #find cells whose texts are not '' qtable
    occupiedCells=[]
    skipCells=[]
    for i in range(table.rowCount()):
        for j in range(table.columnCount()):
            if table.item(i,j).text()=='' or (i,j) in skipCells:
                continue
            rowSpan,columnSpan = table.rowSpan(i,j),table.columnSpan(i,j)
            for k in range(rowSpan):
                for l in range(columnSpan):
                    occupiedCells.append((i+k,j+l))
                    skipCells.append((i+k,j+l))
